...
### 一键写作与润色
- **一键写章**：host 直写（上下文包 → 正文 → 自动落盘），章节统计/账本/审计联动；写作指令强调**严格遵循本章细纲/全书设定/前文事实**，避免自创冲突
- **一键润色**：AI 优化文笔（不改情节/人物/走向），改动拆成**逐条建议**，每条可**单独采纳/拒绝**（也可全部采纳/全部拒绝），只保存你选中的改动
- **字级 diff 标亮**：每条建议只标出**具体改了哪几个字**（删除线=删，黄底/蓝底=新增或改动），不再整段标红
...
### 1️⃣ 创建一本书（九阶段流程的起点）
- **界面**：打开「大肥鱼的小说工坊」抽屉 → 顶部输入**书名** → 选**题材**（27 种，按分组排列）→ 点「创建」。
- **对话**：「创建项目，写本玄幻小说」「新书，仙侠」「开本都市文」。
- 创建后进入项目详情，看到当前阶段（初始为 `topic` 选题）与各阶段状态。

> 也可以先点「**一键导入示例《青云问道》**」，一步得到一本带世界书条目的示例书，边看边学。

### 2️⃣ 走完九阶段（选题→…→完本）
- 阶段链：`选题 → 设定 → 人设 → 大纲 → 分卷 → 细纲 → 正文 → 修订 → 完本`，**前一阶段未批准不能进入下一阶段**（门禁）。
- **怎么推进**：
  - **对话**：说「生成世界观设定」「设计人设」「写全书大纲」，模型会用相应模板产出该阶段文档；完成后说「提交设定」走 `novel_commit` → 校验通过即批准解锁下一阶段。
  - **界面**：目前阶段推进主要通过对话里的 agent 工具完成（`novel_phase` / `novel_commit`），舞台右上角会显示当前阶段与门禁状态。
- 产物自动存 `docs/<阶段>.md` + 留历史版本（`versions/<阶段>/v<N>`），随时可回退。

### 3️⃣ 写正文（一键写章）
- **界面**：项目详情 → 选**章节号** → 点「**一键写章并保存**」→ 模型按「全书设定＋本卷/本章细纲＋前文＋世界书命中等」的上下文包自动写一章 → **已自动存盘**，统计/账本/审计联动。
- **对话**：「帮我写下一章」「写第 5 章」「继续写」。
- 写好后左侧编辑框/B 面板可看到正文与字数（达标徽标）。不满意就点「一键润色」或手动改。

### 4️⃣ 润色与撤销
- **一键润色**：点「**一键润色**」→ 模型优化文笔（**不改情节/人物/走向**）→ 下方「润色预览」用**黄色=润色后文、红色删除线=被替换原文**标出改动处 → 点「确认保存」（写入章节）或「放弃还原」（恢复原文）。
- **撤销**：编辑区上方「↶ 撤销（N）」按钮。手动编辑、写章回填、润色应用都会记录历史，可**连续点击逐步还原**到任意之前的状态。

### 5️⃣ 建世界书（固化设定，不崩设定）
- **入口**：项目详情右下「**本书世界书**」，或抽屉列表里书籍行的「世界书」按钮。
...
### 6️⃣ 导入本地书籍
- **界面**：项目列表「**导入本地书籍**」→ 选一个本地 `.txt` / `.md` 网文文件 → 自动识别章节标题（含"第一章/楔子/番外"等）并建书 → 全部正文按章同步，可直接续写/润色。
- **对话**：「导入本地 txt」。

### 7️⃣ 质量体检（诊断 / 去 AI 味 / 校验）
- **结构诊断**：项目详情底部「**结构诊断**」→「诊断本章」→ 得分与问题（钩子/开场/冲突/灌输/字数/对话）。
- **去 AI 味**：对话「把这段去 AI 味」→ `novel_depolish` 检测并改写（内置 234 词 5 类词库）。
- **四族校验**：提交时自动跑结构/内容/剧情/一致性；也可对话「校验这章」手动触发 `novel_validate`。

### 8️⃣ 保持一致性（长文不崩设定）
- **事实账本**：章节内 `<JSONPatch>` 自动落账（人物境界/物品/地点状态）。
- **巡检**：对话「跑一遍一致性巡检」→ `novel_consistency_audit` 查账本冲突、时间线倒挂、伏笔超期，并给出世界书沉淀建议。
- **查账本**：「林远现在什么境界」「查一下账本」→ `novel_ledger`。
- **伏笔/时间线**：对话「登记个伏笔，第 3 章埋，第 20 章回收」「记录第 5 章书内时间『第三日』」→ `novel_foreshadow` / `novel_timeline`。

### 9️⃣ 修订与导出
- **修订**：对话「修订第 3 章，校错别字」→ `novel_revise`（校对/节奏/文风三模式，产出**新版本不覆盖原稿**）。
- **导出**：界面项目详情「**导出 txt**」（带 BOM，可直接投稿）；对话「导出成稿」→ `novel_export`（txt / markdown / 平台排版）。

### 🔟 市场调研 & 模板复制
- **市场调研**（选题用）：对话「调研下仙侠的市场行情」→ `novel_market_research` **mode=prompt** 给你一组 `web_search` 检索词 → 模型检索后以 **mode=report** 把榜单热词/读者偏好/同类卖点落盘 `reports/market.md` → 选题报告里引用。
- **模板复制**（克隆设定骨架）：对话「以《青云问道》为模板开本新书」→ `novel_clone_project` 复制**字数目标/风格/禁用词 + 已完成的阶段设定文档**（选题/设定/人设/大纲/分卷/细纲），**正文不复制**，状态机重置——直接对着现成设定写正文。

### 1️⃣1️⃣ 灵感 / 术语表
- 对话「记个灵感：雨夜剑冢」「建个术语表：玄天宗——地域宗门」→ `novel_idea` / `novel_glossary`（选题拉取聚合、术语注入防用词漂移）。

### 1️⃣2️⃣ 用「创作模式预设」开新会话
- 新建会话 → 预设选择器选「**大肥鱼的小说工坊**」→ 直接对话「创建项目，写本玄幻小说」即进入创作模式（persona + 技能方法 + 41 个工具自动就位）。

> 💡 **提示**：几乎每个功能都有对应的 `novel_*` 工具，模型在创作模式下会自己调用并提示你确认（写操作）。你不必记命令，**用大白话提需求即可**；想知道有哪些能力，对话里说「小说工坊能做什么」。
